...
fun dateDigitToStr(digital: String): String {
    ...
    return try {
        ...
        if (parts[1].toInt() > 12) throw Exception()
...
fun flattenPhoneNumber(phone: String): String {
    ...
    if (f > 0 || result.length == 1 && result[0] == '+') result = ""
    return result
}
...
fun plusMinus(expression: String): Int {
    if (Regex("""[^-+\s0-9]|([0-9]+\s+[0-9])|([+-]+\s+[+-])""") in expression) throw IllegalArgumentException()
        ...
        when (commands[lastCommand]) {
            ...
            '[' -> {
                if (list[number] == 0) {
                    while (pair >= 0) {
                        lastCommand++
                        if (lastCommand >= commands.length) throw IllegalArgumentException()
            ...
            ']' -> {
                if (list[number] != 0) {
                    while (pair <= 0) {
                        lastCommand--
                        if (lastCommand < 0) throw IllegalArgumentException()
                        find(commands[lastCommand])
                    }
                }
                pair = 0
