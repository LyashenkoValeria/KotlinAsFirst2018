...
class Tests {
    ...
    fun bestLongJump() {
        ...
        assertEquals(2147483647, bestLongJump("2147483647 2147483647 2147483647 % 1 -    2147483647 - 1490862180  1 - - 2147483647 -  470205933  2147483647 1168972386 %   1 - 0 -  %      1  1023585769 -  -      -  0  1511762000 -"))
    ...
    fun computeDeviceCells() {
        ...
        assertThrows(IllegalArgumentException::class.java) { computeDeviceCells(800, "<-+] < << [------<++", 100) }
        assertThrows(IllegalArgumentException::class.java) { computeDeviceCells(2, "-[+-<+--<-+<+>-<---+-+[ <[--<---> --+<-[<<+-><<<+-<-->+<-]>[<<]<+]<+++]-- <>]-]<---+<]+<--<><++->--+<-<<<<- [<--- -+<- +++<[-[++<<>- ->] -", 10000) }
    }
}